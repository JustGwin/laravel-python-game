@extends('layouts.app')
@section('title', 'รายละเอียด: ' . $user->name)
...
@section('content')
<a href="{{ route('admin.dashboard') }}" class="back-link"><i class="fa-solid fa-arrow-left"></i> กลับ Dashboard</a>

<!-- ─── Player Header ────────────────────────────────────────────────── -->
<div class="player-header">
    <div class="player-avatar">{{ mb_substr($user->name, 0, 1) }}</div>
    <div class="player-info">
        <h1>{{ $user->name }}</h1>
        <p>{{ $user->email }} • โรงเรียน: {{ $user->school ?? '-' }} • เข้าร่วมเมื่อ {{ $user->created_at->diffForHumans() }}</p>
    </div>
    <div style="margin-left:auto">
        <form method="POST" action="{{ route('admin.player.reset', $user) }}"
              onsubmit="return confirm('รีเซ็ตคะแนนทั้งหมดของ {{ $user->name }}?')">
            @csrf @method('DELETE')
...
<div class="card" style="padding:20px">
    ...
    <div class="history-row">
        <!-- Rank -->
        <div class="rank-circle">#{{ ($scores->currentPage() - 1) * 10 + $i + 1 }}</div>

        <!-- Main Info -->
        <div>
            <div style="font-size:14px;font-weight:600;margin-bottom:6px">
                เล่นเมื่อ {{ $score->created_at->format('d/m/Y H:i') }}
            ...
            <div class="mini-level-bars">
                @php $ls = $score->level_scores ?? array_fill(0, 6, 0); @endphp
                @for($d = 0; $d < 6; $d++)
                    @php $pts = $ls[$d] ?? 0; @endphp
                    <div
                        class="mini-bar {{ $pts > 0 ? '' : 'empty' }}"
                        style="height: {{ max(8, $pts * 2) }}px"
                        title="ด่าน {{ $d+1 }}: {{ $pts }} คะแนน"
                    ></div>
                @endfor
                <span style="font-size:11px;color:var(--muted);margin-left:6px;align-self:center">{{ $score->levels_completed }}/6 ด่าน</span>
            </div>
        </div>

        <!-- Score -->
        <div style="text-align:center">
            <div style="font-size:22px;font-weight:700;color:var(--good)">{{ $score->score }}</div>
            <div style="font-size:11px;color:var(--muted)">คะแนน</div>
        </div>

        <!-- Grade -->
        <div>
            <span class="chip chip-{{ ['A'=>'green','B'=>'blue','C'=>'yellow','D'=>'yellow'][$score->grade] ?? 'red' }}" style="font-size:16px;padding:5px 14px">
                {{ $score->grade }}
            </span>
        </div>
        ...
        <div style="text-align:right">
            <div style="font-size:14px;margin-bottom:6px">{{ $score->formatted_time }}</div>
            <div style="font-size:12px;color:var(--muted);margin-bottom:8px">ใบ้ {{ $score->hints_used }} ครั้ง</div>
            <div style="display:flex; gap:6px; justify-content:flex-end;">
                <form method="POST" action="{{ route('admin.score.toggle-hide', $score) }}">
                    @csrf @method('PATCH')
                    @if($score->is_hidden)
                ...
                <form method="POST" action="{{ route('admin.score.delete', $score) }}"
