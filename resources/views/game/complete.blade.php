@extends('layouts.app')
@section('title', 'ยินดีด้วย! ผ่านครบทุกด่าน')
...
<div class="complete-wrap">
    ...
    <div class="result-card">
        <div class="big-score">{{ $bestScore->score }}</div>
        ...
        <div class="big-grade">
            ...
            @endphp
            <span style="color:{{ $gradeColors[$bestScore->grade] ?? '#ff5d6c' }}">
                เกรด {{ $bestScore->grade }}
            </span>
        </div>

        <div class="result-grid">
            <div class="result-item">
                <div class="result-val">{{ $bestScore->levels_completed }}/6</div>
                <div class="result-lbl">ด่านที่ผ่าน</div>
            </div>
            <div class="result-item">
                <div class="result-val">{{ $bestScore->formatted_time }}</div>
                <div class="result-lbl">เวลาที่ใช้</div>
            </div>
            <div class="result-item">
                <div class="result-val">{{ $bestScore->hints_used }}</div>
                <div class="result-lbl">คำใบ้ที่ใช้</div>
            </div>
        </div>

        @if($bestScore->level_scores)
        <div class="level-breakdown">
            @foreach($bestScore->level_scores as $i => $pts)
            <div class="level-pill">ด่าน {{ $i+1 }}: {{ $pts }} คะแนน</div>
            @endforeach
        </div>
        @endif
    </div>
    ...
    <div class="btn-row">
        <a href="{{ route('game.index') }}" class="btn btn-primary"><i class="fa-solid fa-rotate-right"></i> เล่นอีกครั้ง</a>
        <a href="{{ route('game.history') }}" class="btn btn-ghost"><i class="fa-solid fa-clock-rotate-left"></i> ดูประวัติ</a>
    </div>
</div>
@endsection
